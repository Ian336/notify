...
export default {
  ...
  methods: {
    isOpenNotify (val) {
      ...
      chrome.runtime.sendMessage({ isOpen: val,notifyTime:this.defaultNotifyTime })
    },
    timeEndChange (val) {
      this.workTime = '敬业时长'
    },
    ...
    getChromeStorage () {
      chrome.storage.local.get(['notifyTime', 'tasks', 'closeNotify', 'isOpen'], (res) => {  
        //不支持?? 
    ...
    getWokeTime () {
      chrome.storage.local.get(['totolWorkTimes'], res => {
        ...
        result = parseInt((result / 3600000)) + 'h' + parseInt(result % 3600000 / 60000) + 'm'
